...
def load_courses():
    data_path = os.path.join(current_app.config['DATA_FOLDER'], 'courses.json')
    try:
        with open(data_path, 'r', encoding='utf-8') as f:
            courses = json.load(f).get('courses', [])
            for c in courses:
                author = get_user_by_id(c['author_id'])
                c['author'] = author['login'] if author else 'Неизвестный автор'
            return courses
    except (FileNotFoundError, json.JSONDecodeError):
        return []
...
def create_course(title, description, author_id):
    courses = load_courses()
    new_course = {
        'id': generate_id(),
        'author': get_user_by_id(author_id)['login'],
        'title': title,
        'description': description,
        'author_id': author_id,
        'created_at': datetime.now().isoformat(),
        'token': generate_id(),  # Уникальный токен для доступа к курсу
...
def get_course_by_token(token):
    courses = load_courses()
    return next((c for c in courses if c['token'] == token), None)