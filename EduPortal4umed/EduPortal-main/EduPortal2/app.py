...
def courses():
    ...
    # Фильтруем курсы, если пользователь не администратор
...
def add_course():
    ...
    if request.method == 'POST':
        ...
        course = create_course(title, description, current_user.id)
        messages.append(('success', f'Курс "{course["title"]}" успешно создан! Токен для доступа: {course["token"]}'))

    return render_template('courses/add_course.html', messages=messages)

@app.route('/courses/join', methods=['GET', 'POST'])
@login_required
def join_course():
    if request.method == 'POST':
        token = request.form.get('token')
        course = get_course_by_token(token)
        if course:
            flash(f'Найден курс: "{course["title"]}". Токен: {course["token"]}', 'success')
        else:
            flash('Курс с таким токеном не найден', 'error')
    return render_template('courses/join_course.html')
...
def course_detail(course_id):
    ...
    # Проверка прав
    if not (current_user.account_type == 'admin' or
            (current_user.account_type == 'teacher' and course['author_id'] == current_user.id)):
